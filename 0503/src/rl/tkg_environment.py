...
class TKGEnvironment(BaseEnvironment):
    ...
    def evaluate_mrr(self, active_selected_samples=None):
        ...
        try:
            # 准备包含历史上下文和主动选择样本的提示
            prompt, candidates = self._prepare_prompt_combined(query_sample, query_direction, self.historical_samples, active_selected_samples)

            # 如果准备提示失败或 candidates 为空，则无法继续
            if not prompt or not candidates:
                 logger.warning("evaluate_mrr: 无法准备提示或候选列表为空，返回0.0")
                 return 0.0, 0, 0

            # 获取搜索空间 (这部分可能不再需要，因为过滤逻辑应该在 update_metric 中处理，但暂时保留)
            ...
            # 使用模型进行预测
            # 修改：接收元组列表
            predictions_tuples = predict(self.reward_tokenizer, self.reward_model, prompt, self.args)

            if not predictions_tuples:
                logger.debug("evaluate_mrr: 模型未返回预测结果，返回0.0")
                return 0.0, 0, 0

            # --- 新增：将 (索引, 分数) 元组列表转换为实体字符串列表 ---
            ranked_entity_strings = []
            for idx, score in predictions_tuples:
                # 检查索引是否在 candidates 列表的有效范围内
                if 0 <= idx < len(candidates):
                    ranked_entity_strings.append(candidates[idx])
                else:
                    logger.warning(f"evaluate_mrr: 预测返回的索引 {idx} 超出候选列表范围 (长度 {len(candidates)})。")
            # --- 转换结束 ---

            logger.debug(f"evaluate_mrr: 预测结果 (字符串) 数量 {len(ranked_entity_strings)}, 前5: {ranked_entity_strings[:5]}")
            ...
            # 修改：遍历转换后的字符串列表
            for i, pred_entity in enumerate(ranked_entity_strings, 1):
                # 修改：直接比较字符串
                if pred_entity in correct_targets_set:
                    rank = i
                    logger.debug(f"evaluate_mrr: 找到正确答案 '{pred_entity}' @ rank {rank}")
                    break # 找到第一个匹配的目标即可确定排名

            # 更新指标 (这里使用 HitsMetric 的逻辑，而不是直接修改 rank)
            # 注意：evaluate_mrr 的目标是计算 MRR，不是应用过滤规则。
            # HitsMetric 内部没有过滤逻辑，所以这里的 rank 计算是 unfilterd rank。
            metric.total += 1
            if rank != float('inf'):
                metric.update(rank) # 使用找到的第一个匹配目标的 rank
                logger.debug(f"evaluate_mrr: 更新指标 rank={rank}, reciprocal={1.0/rank:.4f}")
            # else: # 如果没找到，MRR 贡献为 0，不需要 update
            #     logger.debug(f"evaluate_mrr: 未找到正确答案，MRR 贡献为 0")
    ...
    def _prepare_prompt_combined(self, query_sample, query_direction, historical_samples, active_selected_samples):
        ...
        Args:
            ...
            historical_samples: 历史上下文样本列表 (注意：此参数已不再使用，实际历史来自 self.train_history)
            active_selected_samples: RL Agent选择的当前时间步样本列表

        Returns:
            (str, list): 最终的提示字符串 和 候选实体列表
        """
        # 1. 获取完整格式化提示，直接调用 utils.prepare_input
        try:
            # prepare_input 现在负责处理历史和主动学习样本
            # 修改：接收 prepare_input 返回的 candidates
            model_input_prompt, candidates, _ = prepare_input(
                query_sample,           # 传递样本元组
                self.train_history,     # 传递正确的历史搜索空间
                self.args,
                return_prompt=True,
                active_samples=active_selected_samples # 传递主动选择的样本
            )
            final_prompt = model_input_prompt

        except Exception as e:
            logger.error(f"调用 prepare_input 获取完整提示时出错: {e}", exc_info=True)
            # 修改：返回空字符串和空列表
            return "", [] 

        # 不再需要手动格式化历史或整合主动样本

        # 修改：返回 prompt 和 candidates
        return final_prompt, candidates
    ...
    def reset(self, keep_candidates=False):
        """重置环境
        
        Args:
            keep_candidates: 如果为True，保留当前候选样本池不重置
        """
        # 保存当前查询信息以便在重置后仍可使用
        ...
        # 获取初始样本（如果没有设置current_samples或不保留当前候选样本，使用时间戳）
        if not keep_candidates or not self.current_samples:
    ...
    def action_space(self):
        ...
        for sample, direction in self.current_samples:
            ...
            # --- 添加 NaN 检查 (中间特征) ---
            if torch.isnan(similarity_to_context).any():
                logger.warning(f"NaN detected in action_space for similarity_to_context. Replacing with 0.")
                similarity_to_context = torch.nan_to_num(similarity_to_context, nan=0.0)
            if torch.isnan(similarity_to_query).any():
                logger.warning(f"NaN detected in action_space for similarity_to_query. Replacing with 0.")
                similarity_to_query = torch.nan_to_num(similarity_to_query, nan=0.0)
            if torch.isnan(uniqueness).any():
                logger.warning(f"NaN detected in action_space for uniqueness. Replacing with 0.")
                uniqueness = torch.nan_to_num(uniqueness, nan=0.0)
            # --- 检查结束 ---

            # Combine features: semantic_features + similarity_to_context + similarity_to_query + uniqueness
            action_feature = torch.cat([
                candidate_embedding,
                similarity_to_context.unsqueeze(0), # Ensure tensor shape compatibility
        ...
        if not action_features:
            logger.warning("No valid action features generated.")
            # Return empty tensors if no features
            return torch.empty((0, self.action_feature_dim)), [], []
        
        action_features_tensor = torch.stack(action_features)
        
        # --- 添加 NaN 检查 (最终特征) ---
        if torch.isnan(action_features_tensor).any():
            logger.warning(f"NaN detected in final action_features_tensor. Replacing NaN with 0.")
            action_features_tensor = torch.nan_to_num(action_features_tensor, nan=0.0)
        # --- 检查结束 ---

        return action_features_tensor, entity_ids, relation_ids
    
    def step(self, action_idx, update_candidates=True):
        """
        执行一步动作（选择一个样本）
        
        Args:
            action_idx: 动作索引，对应要选择的样本索引
            update_candidates: 如果为True，从可用候选池中移除已选样本；否则保留候选池不变
        ...
        # 从当前可用样本中移除（如果update_candidates为True）
        ...
        # 检查是否达到最大步数或没有可用样本
        done = self.steps_taken >= self.max_steps or (update_candidates and not self.current_samples)
        if done:
    ...
    def update_candidates(self, samples, indices):
        ...
        logger.info(f"环境样本已更新，新样本数: {len(samples)}")
    ...
    def _extract_context_features(self, use_cache=True):
        ...
        # --- 添加 NaN 检查 ---
        if torch.isnan(projected_embedding).any():
            logger.warning(f"NaN detected in _extract_context_features AFTER projection. Replacing NaN with 0.")
            projected_embedding = torch.nan_to_num(projected_embedding, nan=0.0)
        # --- 检查结束 ---
    ...
    def _extract_interaction_features(self, use_cache=True):
        ...
        if self.feature_config["interaction_method"] == "attention":
            ...
            # 检查标准差是否接近0，若是则替换为小的epsilon值，防止后续计算出现NaN
            if std_score < 1e-8:
                std_score = torch.tensor(1e-8, device=attention_scores.device)
            ...
            # --- 添加对输入嵌入的 NaN 检查 ---
            # 检查查询嵌入
            query_nan = torch.isnan(query_embedding).any()
            # 检查样本嵌入列表（如果非空）
            samples_nan = False
            if sample_embeddings: # 仅在列表非空时检查
                 samples_nan = any(torch.isnan(emb).any() for emb in sample_embeddings)

            if query_nan or samples_nan:
                logger.warning(f"NaN detected in input embeddings for similarity calculation (query_nan={query_nan}, samples_nan={samples_nan}). Returning zero features.")
                # 返回与 attention 分支相同维度的零向量 (16维)
                feature_vector = torch.zeros(16, device=query_embedding.device)
            else:
                # 只有在输入有效时才计算相似度
                # 计算查询与每个样本的余弦相似度
                # 需要处理 sample_embeddings 为空的情况
                if sample_embeddings:
                    similarities = torch.tensor([
                        F.cosine_similarity(query_embedding.unsqueeze(0), embedding.unsqueeze(0), dim=1)[0] # dim=1 for cosine similarity between vectors
                        for embedding in sample_embeddings
                    ], device=query_embedding.device) # 确保在同一设备上
                else:
                    similarities = torch.tensor([], device=query_embedding.device) # 创建空的 similarities 张量
                ...
                if similarities.numel() > 0:
                    ...
                    # std_sim = similarities.std() # <-- 移除标准差计算

                    # 构建基础特征向量 (3维)
                    # 确保统计量也在正确的设备上
                    base_features = torch.tensor([max_sim, min_sim, mean_sim], device=query_embedding.device)

                    # 填充到16维以匹配 attention 分支
                    feature_dim = 16
                    # 确保 padding 在正确的设备上
                    padding = torch.zeros(feature_dim - base_features.shape[0], device=query_embedding.device)
                    feature_vector = torch.cat([base_features, padding])
                else:
                    # 如果没有相似度分数 (例如 sample_embeddings 为空)
                    feature_vector = torch.zeros(16, device=query_embedding.device) # 返回16维零向量
        
        # 缓存结果
        if use_cache:
            self.feature_cache[cache_key] = feature_vector # 确保缓存的是最终的 feature_vector
            
        # --- 添加 NaN 检查 ---
        if torch.isnan(feature_vector).any():
            logger.warning(f"NaN detected in _extract_interaction_features AFTER calculation. Replacing NaN with 0.")
            feature_vector = torch.nan_to_num(feature_vector, nan=0.0)
        # --- 检查结束 ---
    ...
    def _calculate_diversity_feature(self, use_cache=True):
        ...
        # --- 添加 NaN 检查 ---
        if torch.isnan(feature_vector).any():
            logger.warning(f"NaN detected in _calculate_diversity_feature AFTER calculation. Replacing NaN with 0.")
            feature_vector = torch.nan_to_num(feature_vector, nan=0.0)
        # --- 检查结束 ---
